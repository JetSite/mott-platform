...
function buildFromRows(rows: Row[]): Result {
  ...
  const columnToWidth = rows
    ...
    .reduce((columnToLength1, columnToLength2) => {
      const result: Record<string, number> = {};
      for (const column in columnToLength1) {
        result[column] =
          columnToLength1[column] >= columnToLength2[column]
            ? columnToLength1[column]
            : columnToLength2[column];
      }
      return result;
    });
...
function build(columns: Column[], rows: Row[]): Result {
  const result = [getHeaderRow(columns), getLines(columns)];
